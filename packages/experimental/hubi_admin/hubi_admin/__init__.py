...
class Admin:
    ...
    def compile(self, resource, result_set=None):
        ...
        def process_result_set(result_set=None):
            ...
            pprint.pprint(result_set.__dict__)
            return result_set
            ...
            return render(
                """
                    ...
                    {% block content scoped %}
                        {% include 'components/admin/views/ListView.html' ignore missing with context %}
                    {% endblock %}
    ...
    def setup_create_view(self, resource: str):
        ...
        return render(
            ...
            {% block content %}
                <mv-admin.views.CreateView 
                    page_title={{ page_title }}                    
                    resource={{ data_model.resource[0].name }}
                    query_param_string={{ data_model.resource[0].query_param_string }}
                    result_set={{ {} }}
    ...
    def setup_edit_view(self, resource: str, id: int, result_set=None):
        ...
        return render(
            ...
            {% extends 'components/layouts/BaseLayoutAdmin.html' %}
                ...
                {% block content %}
                    {% include 'components/admin/views/EditView.html' ignore missing with context %}
                {% endblock content %}
            """, with_layout=False  # , is_path=True
        )
    ...
    def __generate_models__(self):
        for file in glob.glob("application/models" + "/*.py"):
            ...
            if name == "__init__":
                ...
                model = to_class("application.models." + name + "." + name)
                display_in_nav = True
                try:
                ...
                if display_in_nav:
                    self.models.append({
                        "name": name,
                        "class": to_class("application.models." + name + "." + name),
                    })
        return self.models
